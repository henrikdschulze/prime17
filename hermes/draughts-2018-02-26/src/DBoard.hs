{-|
  This module exports all the functions required by
  'playDraughts' function in 'Draughts' game.
-}
module DBoard(
  Square(Square),
  Player(Red, White),
...
data Player = Red -- ^ To represent 'Red' 'Player'
  | White -- ^ To represent 'White' 'Player'
    deriving (Eq) -- Equable
...
-- | Test if the given 'Piece' is 'White'
isWhite :: Piece -> Bool
isWhite Empty = False
isWhite (Piece pl _) = pl == White
...
-- ^ A 'Game' consists of the 'BoardMap', possible moves of the current 'Player',
-- the current 'Player' and the 'Square' which was clicked
...
startGame = addPieces (BoardMap (Map.empty)) squares
  where squares = [(Square x y) | x <- [1..side], y <- [1..side], (mod x 2 == 0 && mod y 2 == 1) || (mod x 2 == 1 && mod y 2 == 0)]
...
initialPieceAtSquare (Square x y) | y <= (side`div`2 - 1) = (Piece Red Pawn)
                                  | y >= (side`div`2 + 2) = (Piece White Pawn)
                                  | otherwise = Empty
...
-- During Jump remove the current piece at the current positon, add it at the
-- final position and remove the pieces jumped over. Check for promotion to King:
mPlayMove board (Jump s1@(Square x1 y1) s2@(Square x2 y2))
  | y2 /= side && y2 /= 1 = replacePiece (replacePiece (replacePiece board s2
    (seekBoard board s1)) s1 Empty) (Square (quot (x1+x2) 2) (quot (y1+y2) 2)) Empty
  | otherwise = replacePiece (replacePiece (replacePiece board s2
    (promote $ seekBoard board s1)) s1 Empty) (Square (quot (x1+x2) 2) (quot (y1+y2) 2)) Empty
-- During Move remove the current piece at the current positon,
-- add it at the final position. Check for promotion to King:
mPlayMove board (March s1@(Square x1 y1) s2@(Square x2 y2))
  | y2 /= side && y2 /= 1 = replacePiece (replacePiece board s2 (seekBoard board s1)) s1 Empty
...
mValidMoves b pl
  | length jumpsList > 0 = jumpsList -- If jump is possible then only jumps are allowed
...
mValidJumps b (s@(Square x y), piece@(Piece player ptype))
  | player == White && ptype == Pawn && y >= (side`div`2 - 1) = validJumpsForWhitePawn b s
  | player == White && ptype == King = validJumpsForWhiteKing b s
  | player == Red && ptype == Pawn && y <= (side`div`2 + 2) = validJumpsForRedPawn b s
  | player == Red && ptype == King = validJumpsForRedKing b s
  | otherwise = []
mValidJumps _ (_, Empty) = []

-- | Get valid jumps for the passed White Pawn
validJumpsForWhitePawn :: BoardMap -> Square -> [Move]
-- Check if it is possible to Jump in the two directions, if so add to the List
validJumpsForWhitePawn board s@(Square x y) = [Jump (Square x y) (Square a b)
  | (a, b) <- [(x-2, y-2), (x+2, y-2)], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b)
  == Empty, isRed $ seekBoard board (Square (quot (a+x) 2) (quot (b+y) 2))]

-- | Get valid jumps for the passed Red Pawn
validJumpsForRedPawn :: BoardMap -> Square -> [Move]
-- Check if it is possible to Jump in the two directions, if so add to the List
validJumpsForRedPawn board s@(Square x y) = [Jump (Square x y) (Square a b)
  | (a, b) <- [(x-2, y+2), (x+2, y+2)], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b)
  == Empty, isWhite $ seekBoard board (Square (quot (a+x) 2) (quot (b+y) 2))]

-- | Get valid jumps for the passed White King
validJumpsForWhiteKing :: BoardMap -> Square -> [Move]
-- Check if it is possible to Jump in the four directions, if so add to the List
validJumpsForWhiteKing board s@(Square x y) = [Jump (Square x y) (Square a b)
  | a <- [x-2, x+2], b <- [y-2, y+2], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b)
  == Empty, isRed $ seekBoard board (Square (quot (a+x) 2) (quot (b+y) 2))]

-- | Get valid jumps for the passed Red King
validJumpsForRedKing :: BoardMap -> Square -> [Move]
-- Check if it is possible to Jump in the four directions, if so add to the List
validJumpsForRedKing board s@(Square x y) = [Jump (Square x y) (Square a b)
  | a <- [x-2, x+2], b <- [y-2, y+2], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b)
  == Empty, isWhite $ seekBoard board (Square (quot (a+x) 2) (quot (b+y) 2))]
...
mValidMarches b (s@(Square x y), piece@(Piece player ptype)) | player == White && ptype == Pawn && y >= 2 = validMarchesWhitePawn b s -- Get valid marches for White Pawn
...
validMarchesKing board s@(Square x y) = [March (Square x y) (Square a b) | a <- [x-1, x+1], b <- [y-1, y+1], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b) == Empty]

-- | Get valid marches for the passed White Pawn
validMarchesWhitePawn :: BoardMap -> Square -> [Move]
-- Check if it is possible to March in the two directions, if so add to the List
validMarchesWhitePawn board s@(Square x y) = [March (Square x y) (Square a b) | (a, b) <- [(x-1, y-1), (x+1, y-1)], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b) == Empty]

-- | Get valid marches for the passed Red Pawn
validMarchesRedPawn :: BoardMap -> Square -> [Move]
-- Check if it is possible to March in the two directions, if so add to the List
validMarchesRedPawn board s@(Square x y) = [March (Square x y) (Square a b) | (a, b) <- [(x-1, y+1), (x+1, y+1)], a >= 1 && a <= side && b >= 1 && b <= side, seekBoard board (Square a b) == Empty]
